...
class Bintree:
    ...
    def put(self,newvalue):                             # Metod som lägger in ett nytt värde i trädet, tar newvalue som indata
        self.root = putta(self.root,newvalue)

    def __contains__(self,value):                       # Metod som letar upp ett värde i trädet, tar value som indata
...
def putta(nod, newvalue): 
    if nod == None:                                     # Om noden inte finns är trädet tomt och den nya noden blir root
        return Node(newvalue)
    if newvalue == nod.value:                           # Om värdet på noden och det nya värdet är lika returnerar vi noden (lämnar oförändrard)
        return nod
    if newvalue < nod.value:                            # Om det nya värdet är mindre än det värdet i noden, kallar rekursivt och uppdaterar den nya positionen 
        nod.left = putta(nod.left,newvalue)
        return nod
    if newvalue > nod.value:                            # Om det nya värdet är större än det värdet i noden, kallar rekursivt och uppdaterar den nya positionen 
...
def finns(nod,value):
    if nod == None:                                     # Om noden inte finns finns inget i trädet, returnerar falskt 
        return False
    if value == nod.value:                              # Om värdet är samma som värdet på noden finns värdet, returnerar falskt
        return True
